...
with tqdm(total=total_pairs, desc="Calculating cCRE overlaps") as pbar:
    for i, node1 in enumerate(node_dir):
        ...
        node1_bed = os.path.join(temp_folder, f"{node1_name}.bed")

        ccre = configs['ccre_bed']
            
        # Create output file for the intersection
        intersect_bed = os.path.join(temp_folder, f"{node1_name}_intersect.bed")
        ...
        try:
            ...
            df_int.to_csv(os.path.join(out_dir, f"{node1_name}_{seq_counts[node1]}.csv"), index=None)
...
with tqdm(total=total_pairs, desc="Calculating RMSK overlaps") as pbar:
    for i, node1 in enumerate(node_dir):
        ...
        try:
            df_int = pd.read_csv(intersect_bed, sep='\t', index_col=None, header=None)
            df_int.to_csv(os.path.join(out_dir, f"{node1_name}_TE_{seq_counts[node1]}.csv"), index=None)
...
for i, node1 in tqdm(enumerate(node_dir)):
    ...
    try:
        df_int = pd.read_csv(os.path.join(out_dir, f"{node1_name}_1000.csv"), index_col=None, header=0)
        for el in elems:
            df_int_ = df_int[df_int['11']==el]
            pct_states[node1_name].append(len(df_int_))
...
df_states = (df_states - df_states.mean(axis=0))/df_states.max(axis=0)
df_states.fillna(0, inplace=True)
df_states.to_csv(os.path.join(model_path, 'model_cCREs_intersect.csv'))
...
for i, node1 in tqdm(enumerate(node_dir)):
    ...
    try:
        df_int = pd.read_csv(os.path.join(out_dir, f"{node1_name}_TE_1000.csv"), index_col=None, header=0)
        for el in elems:
            df_int_ = df_int[df_int['11']==el]
            pct_states[node1_name].append(len(df_int_))
    except FileNotFoundError:
...
df_rmsk = (df_rmsk - df_rmsk.mean(axis=0))/df_rmsk.max(axis=0)
