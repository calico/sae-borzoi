...
mkdir -p "${OUTPUT_DIR}/${DATE_STAMP}/tomtom_test_archetype_rna_out"
    ...
    if [ -f "$fa_file" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            ${MEME_INSTALL_DIR}/src/tomtom \
                ...
                "${MEME_INSTALL_DIR}/motif_databases/archetype_motifs_RNA.meme" \
                -o "${OUTPUT_DIR}/${DATE_STAMP}/tomtom_test_archetype_rna_out/${filename}"
...
TomTom results: ${OUTPUT_DIR}/${DATE_STAMP}/tomtomtomtom_test_archetype_rna_out_out/
EOF
