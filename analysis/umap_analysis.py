...
def process_meme_results(model_folder, configs):
    ...
    for ind,node_id in tqdm(enumerate(node_folders)):
        ...
        if os.path.exists(os.path.join(model_path, 'tomtom_test_archetype_out', node_id, 'tomtom.tsv')) == False:
            print(f"Motif file for node {node_id} does not exist.")

        output_file = os.path.join(model_path, 'tomtom_test_archetype_out', node_id, 'tomtom.tsv')
        try:
