...
if [ $# -ne 2 ]; then
    echo "Usage: $0 <input_directory> <db_directory>"
    echo "Input directory should contain MEME PWM files"
    exit 1
fi
...
for filename in "${INPUT_DIR}"/meme_test_out/*; do
    ...
    if [ -f "${INPUT_DIR}/tomtom_self_out/${basename}/tomtom.tsv" ]; then
    ...
    ${MEME_INSTALL_DIR}/src/tomtom \
        "${filename}/meme.txt" \
        "${DB_DIR}" \
        -o "${INPUT_DIR}/tomtom_self_out/${basename}"
